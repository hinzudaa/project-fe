...
export default function LoginPage() {
  ...
  const handleCredentialSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await authApi.loginWithCredentials({ username, password });
      if (res.token) setAuthToken(res.token);
      loginUser(res.user);
      router.push("/");
    } catch (err) {
          ...
          {step === "phone" ? (
            <div className="pane-in liquid-glass-card rounded-[28px] p-7 md:p-9 space-y-6">
              ...
              <header className="space-y-2">
                ...
                <p className="text-white/55 text-sm leading-relaxed">
                  {method === "phone"
                    ...
                    : "Нэвтрэх нэр, нууц үгээ оруулж нэвтэрнэ үү."}
                </p>
              </header>
              ...
              <p className="text-[13px] text-white/55 text-center">
                Бүртгэлгүй юу?{" "}
                <Link href="/register" className="text-white hover:underline underline-offset-2 transition-colors">
                  Бүртгүүлэх
                </Link>
              </p>
